      ...
      <div class="p-5">
        ...
        <form class="user" action="{{ route('password.update') }}" method="POST">
          ...
          <div class="form-group">
            <input type="email" class="form-control form-control-user @error('email') is-invalid @enderror" name="email" value="{{ $email ?? old('email') }}" id="exampleInputEmail" aria-describedby="emailHelp" placeholder="Enter Your Email Address">
            {{-- pesan error --}}
            @error('email')
